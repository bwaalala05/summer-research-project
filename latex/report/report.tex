...
\begin{document}
	...
	We have also performed regression analysis on built-in datasets such as California housing and diabetes dataset in \texttt{scikit-learn} and tried fitting highly correlated models. Subsequently, ridge regression models and lasso models were also used to examine its effectiveness in decreasing the standard errors of these models.
	...
	For ridge regression and lasso, otherwise known as $L_2$ and $L_1$ regularisation respectively, we find that it successfully decreases standard errors at the cost of increasing the bias (and hence decreasing $R^2$ score compared to the OLS model) by a neglibible amount. However, in the face of perfect collinearity, ridge and lasso does not yield interpretable coefficients, where the correlated predictors have equally distributed coefficients, as in the case of OLS due to the effect of the scaler. However, there is no room for argument in this case as ridge and lasso explicitly requires that the predictors are standardised.
	...
	\subsection{Effect of \texttt{StandardScaler} on collinear columns in data matrix}
